...
def main():
    ...
    if st.button("이슈 가져오기"):
        ...
        for index, link in enumerate(naver_news_links):
            ...
            summarized_content += fetch_from_openai("gpt-4", [
                {"role": "user",
                 "content": f"{crawled_article['title']} 및 {crawled_article['content']} 내용들을 잘 정리해서 신문 기사 스타일의 보고 자료를 만들어. 다루는 공통된 내용과 공통되지 않은 내용 모두 포함해 전체 내용이 잘 드러나는 기사 스타일의 보고 자료로 만들거야. 누가, 언제, 어디서, 무엇을, 어떻게, 왜 등 내용을 모두 포함시키고 숫자를 정확하게 확인해. '눈길을 끌었다' '주목된다' 등 판단이나 창의적인 내용들은 빼고 2500자 이내로 써 줘. 문장의 마무리는 늘 '했다' '됐다' '줬다' '였다' 처럼 끝내야 해. 내용 중에 [] 이 대괄호나 = 같은 부호가 들어가지 않게 해줘."}
            ], spinner_text)
    ...
    if st.button("생성하기"):
        ...
        st.session_state.final_article_content = fetch_from_openai("gpt-4", [
            {"role": "user",
             "content": f"이 리포트({st.session_state.summarized_content})를 토대로 신문 기사를 쓸거야. 정리된 리포트 내용과 조금 다른 문장 구조로 1000자 이내로 기사를 써 줘. 특히 숫자와 관련된 내용을 다룰 때 틀리지마. {st.session_state.prompt}에 써놓은 기사 시작문으로 문장을 시작해. 리포트 중간에 있는 내용으로 기사 시작문을 시작하는 경우 거의 그대로 써 줘. '~했다' '~됐다'와 같은 반말로 써."}
