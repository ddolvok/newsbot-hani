...
def crawl_and_get_article(url, index, session_id):
    crawled_article = {}
    ...
    with open(f'crawled_article_{session_id}_{index}.json', 'w') as f:
...
def main():
    ...
    if st.button("이슈 가져오기"):
        ...
        for index, link in enumerate(naver_news_links):
            if crawled_count >= 3:
                break
            crawled_article = crawl_and_get_article(link, index + 1, st.session_state.session_id)
            ...
            summarized_content += fetch_from_openai("gpt-4", [
                {"role": "user",
                 "content": f"{crawled_article['title']} 및 {crawled_article['content']} 내용들을 잘 정리해서 신문 기사 스타일의 보고 자료를 만들어. 다루는 공통된 내용과 공통되지 않은 내용 모두 포함해 전체 내용이 잘 드러나는 기사 스타일의 보고 자료로 만들거야. 키워드, 숫자 등을 잘 확인해. '눈길을 끌었다' '주목된다' 등 판단이나 창의적인 내용들은 빼고 2500자 이내로 써 줘."}
            ], spinner_text)
    ...
    if st.button("생성하기"):
        if len(prompt) <= 10:
            ...
            with st.spinner('GPT-4가 기사로 만들고 있어요. 잠시 기다려주세요!'):
                article_content = fetch_from_openai("gpt-4", [
                    {"role": "user",import streamlit as st
import requests
from bs4 import BeautifulSoup
import re
import time
    ...
    if st.button("생성하기"):
        if len(prompt) <= 10:
            ...
            with st.spinner('GPT-4가 기사로 만들고 있어요. 잠시 기다려주세요!'):
                article_content = fetch_from_openai("gpt-4", [
                    {"role": "user",
                     "content": f"{st.session_state.summarized_content} 를 토대로 신문 기사를 쓸거야. 650자에서 1500자 내로 알아서 기사를 써 줘. 특히 숫자와 관련된 내용은 모두 나오도록 해 줘. 기사처럼 줄바꿈을 특히 잘 활용해. 앞서 작성한 리드문 '{prompt}'을 포함하도록 만들어줘."}
                ], '잠시만 기다려주세요!')
                ...
                if st.session_state.article_content:
                    st.download_button(
                        label="다운로드",
                        data=st.session_state.article_content("utf-8"),
