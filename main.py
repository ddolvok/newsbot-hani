...
def main():
    ...
    if st.button("생성하기"):
        ...
        st.session_state.final_article_content = fetch_from_openai("gpt-4", [
            {"role": "user",
             "content": f"이 리포트({st.session_state.summarized_content})를 토대로 신문 기사를 쓸거야. 정리된 리포트 내용과 조금 다른 문장 구조로 1200자 내로 기사를 써 줘. 특히 숫자와 관련된 내용을 다룰 때 틀리지마. {st.session_state.prompt}에 써놓은 리드문으로 문장을 시작해서 내용을 완성해. 리드문과 관련된 내용들을 중심으로 기사를 써 줘. '~했다' '~됐다'와 같은 반말로 써. 내용 중에 [] 이 대괄호나 = 같은 부호가 들어가지 않게 해줘."}
