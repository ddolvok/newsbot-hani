...
def main():
    ...
    if st.button("생성하기"):
        ...
        st.session_state.final_article_content = fetch_from_openai("gpt-4", [
            {"role": "user",
             "content": f"이 리포트({st.session_state.summarized_content})를 토대로 신문 기사를 쓸거야. 정리된 리포트 내용과 조금 다른 문장 구조로 1000자 이내로 기사를 써 줘. 특히 숫자와 관련된 내용을 다룰 때 틀리지마. {st.session_state.prompt}에 써놓은 기사 시작문으로 문장을 시작해. 기사 시작문과 관련된 내용으로만 기사 초안을 만들어 줘. '~했다' '~됐다'와 같은 반말로 써."}
